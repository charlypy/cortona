        ...
        if (hub == 'consumerHub') {
            ...
            proxies['Consumer'].client = {
                onReceiveMessage: function (callback) {
                    return proxies['Consumer'].on("ReceiveMessage", callback);
                },
                registerMessageCallback: function (messageKey, callback) {
                    return proxies['Consumer'].client.onReceiveMessage(function (message) {
                ...
                registerStateObjectLink: function (sentinelName, packageName, name, type, callback) {
                    var result = proxies['Consumer'].client.onUpdateStateObject(function (stateObject) {
                        if (callback && 
                            (stateObject.SentinelName == sentinelName || sentinelName == "*") &&
                            (stateObject.PackageName == packageName || packageName == "*") &&
                            (stateObject.Name == name || name == "*") &&
                            (stateObject.Type == type || type == "*"))
                        {
                            callback(stateObject);
                        }
                    });
                    proxies['Consumer'].server.requestSubscribeStateObjects(sentinelName, packageName, name, type);
                    return result;
                },
            };
            proxies['Consumer'].server = {
                ...
                sendMessageWithSaga: function (scope, key, data, callback) {
                    ...
                    proxies['Consumer'].client.onReceiveMessage(function (message) {
                        if (callback && message.Scope.SagaId == scope.SagaId) {
                            callback(message);
                        }
                    });
